...
def create_acc_commands(packer, enabled, accel_raw, accel_val, upper_jerk, idx, hud_control, set_speed, stopping,
                        long_override, use_fca, cruise_available):
  commands = []
  ...
  scc12_values = {
    ...
    "aReqRaw": accel_raw,
    "aReqValue": accel_val,  # stock ramps up and down respecting jerk limit until it reaches aReqRaw "CR_VSM_Alive": idx % 0xF,
  ...
  scc14_values = {
    ...
    "JerkLowerLimit": 3.0,
    # stock usually is 0.5 but sometimes uses higher values    "ACCMode": 2 if enabled and long_override else 1 if enabled else 4, # stock will always be 4 instead of 0 after first disengage
